...
gar_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern gar_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern gar_intg line; /* source code line number - defined in scanner.c */
...
extern gar_void startParser(boa_void);
extern gar_intg startScanner(ReaderPointer sc_buf);

static gar_void printParserError(gar_char* fmt, ...);
static gar_void displayParser(ReaderPointer ptrBuffer);
static gar_long getParserFileSize(gar_char* fname);
static gar_void callGarbageCollector(boa_void);
...
gar_intg mainParser(gar_intg argc, gar_char** argv) {

	FILE* fi;       /* input file handle */
	gar_intg loadsize = 0; /* the size of the file loaded in the buffer */
...
gar_void printParserError(gar_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(gar_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
gar_long getParserFileSize(gar_char* fname) {
	FILE* input;
	gar_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
gar_void displayParser(ReaderPointer ptrBuffer) {
...
gar_void callGarbageCollector(boa_void) {
