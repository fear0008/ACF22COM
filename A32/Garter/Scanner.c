...
gar_intg line;								/* Current line number of the source code */
extern gar_intg errorNumber;				/* Defined in platy_st.c - run-time error number */

extern gar_intg stateType[];
extern gar_char* keywordTable[];
extern PTR_ACCFUN finalStateTable[];
extern gar_intg transitionTable[][TABLE_COLUMNS];
...
Token tokenizer(void) {
	...
	gar_char c;					/* input symbol */
	gar_intg state = 0;			/* initial state of the FSM */
	gar_intg lexStart;			/* start offset of a lexeme in the input char buffer (array) */
	gar_intg lexEnd;			/* end offset of a lexeme in the input char buffer (array)*/

	gar_intg lexLength;			/* token length */
	gar_intg i;					/* counter */
	gar_char newc;				/* new char */
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((gar_intg)lexLength + 2, 0, MODE_FIXED);
			if (!lexemeBuffer) {
...
gar_intg nextState(gar_intg state, gar_char c) {
	gar_intg col;
	gar_intg next;
	col = nextClass(c);
...
Token funcIL(gar_char lexeme[]) {
	...
	if (lexeme[0] != '\0' && strlen(lexeme) > NUM_LEN) {
		...
		if (tlong >= 0 && tlong <= SHRT_MAX) {
			...
			currentToken.attribute.intValue = (gar_intg)tlong;
		}
		else {
			currentToken = (*finalStateTable[ESNR])(lexeme);
		}
...
Token funcID(gar_char lexeme[]) {
	...
	gar_char lastch = lexeme[length - 1];
	gar_intg isID = GAR_FALSE;
	/* remove the last character read from the lexeme and retract */
	lexeme[length - 1] = '\0';
	readerRetract(sourceBuffer);
...
Token funcSL(gar_char lexeme[]) {
	Token currentToken = { 0 };
	gar_intg i = 0, len = (gar_intg)strlen(lexeme);
...
gar_void printToken(Token t) {
	extern gar_char* keywordTable[]; /* link to keyword table in */
	switch (t.code) {
	case RTE_T:
	...
	case VNID_T:
		...
		break;
	case STR_T:
		printf("STR_T\t\t%d\t ", (gar_intg)t.attribute.codeType);
		printf("%s\n", readerGetContent(stringLiteralTable, (gar_intg)t.attribute.codeType));
		break;
	case INL_T:
