 ...
 * -------------------------------------------------------------
 */
gar_void bErrorPrint(gar_char* fmt, ...);
gar_void displayBuffer(BufferReader* ptr_Buffer);
gar_long getFileSize(gar_char* fname);
gar_intg isNumber(const gar_char* ns);
gar_void startReader(gar_char*, gar_char*, gar_char, gar_intg, gar_intg);
...
gar_intg mainReader(gar_intg argc, gar_char** argv) {

	/* Create source input buffer */
	gar_char* program = argv[0];
	gar_char* input = argv[2];
	gar_char mode = MODE_FIXED;
	gar_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
gar_void startReader(gar_char* program, gar_char* input, gar_char mode, gar_intg size, gar_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	gar_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	gar_char symbol;			/* symbol read from input file */
